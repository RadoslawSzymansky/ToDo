...
function updateList(array = list) {
    ...
    currentArray.forEach((e, index) => {
        ...
        UL.innerHTML += `<li>${index+1}. ${e}<button data-key="${index}">Usuń</button></li>`
    })
...
const addToList = (e) => {
    ...
    if (!task || task.length < 5) return alert('Zadanie musi sie skladac przynajmiej z 5 liter')
    list.push(task)
    updateList();
    INPUT.value = ""
}
...
function filterList(e) {
    ...
    let current = list.map(e => e = e.toLowerCase()).filter(e => e.includes(txt));
